...
2. **Planning gate:** agree on guardrails, definition of done, major decisions,
   and small implementation slices.
...
6. **Publish:** commit intentionally, push, use a draft PR while incomplete,
   merge validated slices, then start the next branch from updated `main`.
7. **Release gate:** declare a milestone complete only when its combined
   definition of done passes—not merely because its last branch merged.
