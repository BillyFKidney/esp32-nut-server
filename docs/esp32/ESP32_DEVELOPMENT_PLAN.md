...
| `v2.8.Z` | `review/wifi-portal-lifecycle` | Standing-approved scanner scope: extract only the fallback captive-portal lifecycle from `wifi.c`, preserving AP+STA/open-AP/DHCP-DNS behavior, critical-section/event-bit ordering, failure cleanup, task lifetime, and all station/credential/physical recovery behavior. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
