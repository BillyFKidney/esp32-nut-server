...
| `v2.8.18` | Release preparation | [OTA reboot-scheduling maintenance](ESP32_OTA_REBOOT_SCHEDULING_CONTRACT.md): ensure a verified image selected for boot is not left pending if the delayed reboot task cannot be created, while preserving OTA responses, NVS result semantics, partition selection, image validation, authorization, and check-only behavior. Independent review, clean candidate build, pinned scoped OTA, service-boundary checks, and healthy NUT acceptance passed; exact-tag build, 3Dprinter acceptance, and publication are authorized. |
