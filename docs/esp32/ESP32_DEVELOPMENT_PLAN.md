...
| `v2.9.0` | Released | [PSRAM management response buffers](../archive/v2.9.0/evidence.md): explicitly placed the bounded 7,000-byte status JSON response and 24-entry full-log snapshot (about 5.2 KiB) in PSRAM, preserving response bytes, authorization, error behavior, zeroization, and service boundaries. Exact-tag ESP-IDF v6.0.2 build/size, authenticated status/full-log and authorization acceptance, scoped 3Dprinter OTA, service-boundary checks, and healthy NUT acceptance passed before [publication](https://github.com/BillyFKidney/esp32-nut-server/releases/tag/v2.9.0). |
| `v2.9.1` | `feature/psram-wifi-scan-records` | PSRAM Wi-Fi scan records: explicitly place the bounded temporary `wifi_ap_record_t` arrays in both portal and authenticated scan paths in PSRAM, with an internal fallback only when PSRAM allocation is unavailable. Preserve the 20-record limit, ordering/deduplication, scan-list cleanup, provisioning and management payloads. Require target proof that `esp_wifi_scan_get_ap_records()` accepts the selected capability, scan/error-path acceptance, exact-target size evidence, and normal service/NUT acceptance. |
