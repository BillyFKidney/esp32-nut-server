...
| Deferred | `review/inherited-compatibility-debt` | The fresh `/src` review confirms ten inherited USB/HID/common/token-store structural items that require dedicated storage, protocol, hardware, or upstream-compatibility contracts. They are intentionally `SKIP`, not candidates for an unscoped cleanup release. |
