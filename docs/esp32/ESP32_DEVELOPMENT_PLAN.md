...
| `v2.9.3` | Released | [PSRAM management-log retention](../archive/v2.9.3/evidence.md): moved the 50-entry, 10,800-byte retained ring to strict PSRAM and removed the 24-entry, 5,184-byte static internal ring. Hot pending-line, lock, and index metadata stay internal; a mutex replaces the `portMUX` before PSRAM access. The exact-tag artifact passed 50-entry full-log, unchanged six-entry status-log, scoped OTA, runtime-heap, service-boundary, and post-reboot NUT acceptance on 3Dprinter. 100-entry retention remains deferred pending a dedicated retrieval/capture soak. |
