...
| `v2.9.4` | Released | [PSRAM auth-state and lock audit](../archive/v2.9.4/evidence.md): target evidence found only about 172 bytes of persistent session/login state (144-byte secret session, two 8-byte locks, and 12 bytes of counters), all on hot authorization paths. It remains internal; API-token stores remain NVS-backed and operation-local. This is an explicit no-move decision that preserves ADMIN/CSRF, token scopes, constant-time comparison, zeroization, and error behavior. |
| `v2.9.5` | `feature/psram-hid-descriptor-contract` | Candidate — [HID descriptor placement audit](../archive/v2.9.5/evidence.md): the maximum 500-entry `HIDData_t` array is 42,000 bytes, but it survives parsing and its entries are cached for recurring USB quick/full polling. Keep the ordinary allocator policy rather than impose strict PSRAM on this hot, long-lived structure; parser scratch and USB transfer/DMA buffers remain internal. Require exact-tag build/size, scoped 3Dprinter OTA, USB-HID full-poll, and service recovery acceptance before publication. |
