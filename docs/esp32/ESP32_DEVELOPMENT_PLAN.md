...
| Deferred | `review/inherited-compatibility-debt` | The fresh `/src` review confirms twenty-two inherited USB/HID/common/token-store/server compatibility-debt findings that require dedicated storage, protocol, hardware, or upstream-compatibility contracts. They are intentionally `SKIP`, not candidates for an unscoped cleanup release. |
