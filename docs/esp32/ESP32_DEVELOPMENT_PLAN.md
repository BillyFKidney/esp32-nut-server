...
| `v2.8.18` | Released | [OTA reboot-scheduling maintenance](../archive/v2.8.18/evidence.md): ensures a verified image selected for boot is not left pending if the delayed reboot task cannot be created, preserving OTA responses, NVS result semantics, partition selection, image validation, authorization, and check-only behavior. Independent review, exact-tag build, scoped 3Dprinter OTA, service-boundary checks, and healthy NUT acceptance passed before [publication](https://github.com/BillyFKidney/esp32-nut-server/releases/tag/v2.8.18). |
| `v2.9.0` | `feature/psram-management-response-buffers` | PSRAM management bulk responses: explicitly place the bounded 7,000-byte status JSON response and the 24-entry full-log route snapshot (about 5.2 KiB at the ESP32-S3 ABI) in PSRAM, retaining internal task stacks and lock-protected log ring. Preserve response bytes, ADMIN/CSRF and bearer authorization, error behavior, zeroization where applicable, and service boundaries. Require exact-target build/size evidence, authenticated status/full-log and unauthorized/invalid-CSRF acceptance, concurrent browser polling, and a healthy full NUT poll. |
| `v2.9.1` | `feature/psram-wifi-scan-records` | PSRAM Wi-Fi scan records: explicitly place the bounded temporary `wifi_ap_record_t` arrays in both portal and authenticated scan paths in PSRAM, with an internal fallback only if the Wi-Fi driver rejects external memory. Preserve the 20-record limit, ordering/deduplication, scan-list cleanup, provisioning and management payloads. Require target proof that `esp_wifi_scan_get_ap_records()` accepts the selected capability, scan/error-path acceptance, exact-target size evidence, and normal service/NUT acceptance. |
| `v2.9.2` | `feature/psram-staged-ota` | PSRAM staged OTA: browser firmware upload writes the complete bounded image to PSRAM, validates it there, and leaves both OTA slots untouched until the ADMIN user explicitly clicks **Install Firmware**. The install action may copy only the verified staged image to the inactive slot, select it for boot, and restart; failed, abandoned, malformed, or unverified uploads must leave both OTA slots valid. Define staged-image lifetime, replacement/expiry, memory-pressure failure, browser reconnect, Agent-route semantics, and zeroization; retain ADMIN/CSRF and scoped-agent authorization. Require malformed/upload/interruption/replacement acceptance, no-write-before-install proof, scoped OTA, rollback/recovery, exact-target size/runtime-heap evidence, and post-reboot service/NUT acceptance. |
| `v2.9.3` | `feature/psram-management-log-retention` | PSRAM management-log retention: move only log-entry payload storage to PSRAM and increase retained capacity to 50, not 100, unless a target soak shows the larger critical-section and retrieval cost acceptable. Keep lock/index state internal; redesign synchronization before any external-memory access occurs under a `portMUX`. Preserve log capture, response ordering, status window, ADMIN authorization, and bounded retrieval behavior. Require capture/polling soak, 50-entry full-log and six-entry status acceptance, exact-target size/runtime-heap evidence, and healthy service/NUT acceptance. |
| `v2.9.4` | `feature/psram-auth-state-audit` | PSRAM auth-state and lock audit: move only infrequently accessed, non-secret bulk state proven safe for PSRAM. Keep session cookie/CSRF state, login counters, `portMUX_TYPE` objects, and hot authorization-path buffers internal unless profiling and API contracts prove otherwise. Record the retained-internal rationale and runtime memory evidence; preserve ADMIN/CSRF, token scope, constant-time comparison, zeroization, and error behavior. |
| `v2.9.5` | `feature/psram-hid-descriptor-contract` | PSRAM HID descriptor contract: make PSRAM placement explicit for the initial bounded 500-entry `HIDData_t` descriptor array, which currently crosses the allocator's 16 KiB internal-preference threshold implicitly. Keep parser scratch and all USB transfer/DMA buffers internal. Preserve descriptor parsing, supported UPS polling, disconnect/reconnect, and read-only behavior; require target allocation evidence, USB-HID full-poll soak, and recovery/service acceptance. |
