...
| `v2.8.Z` | `review/api-token-storage-contract` | Version number remains reserved until this next code-review slice is complete. Refactor only the duplicated OTA/diagnostic token-store lifecycle behind the locked [storage contract](ESP32_API_TOKEN_STORAGE_CONTRACT.md), with no cross-scope authorization or NVS compatibility change. Then complete remaining operational-management acceptance. |
