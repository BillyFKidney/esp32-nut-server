...
| `v2.8.Z` | `review/management-log-levels` | Pending fixer approval: centralize the duplicated private log-level mapping used by runtime-log capture and full-log routes, while preserving every response name and payload contract. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
