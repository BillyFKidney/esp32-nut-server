...
| `v2.8.Z` | `review/token-creation-workflow` | Standing-approved scanner scope: consolidate only the duplicate private API/diagnostic token-creation workflow. Preserve separate stores/scopes/capacities, ADMIN/CSRF, time gate, one-time plaintext responses, metadata/persistence, error texts, and zeroization. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
