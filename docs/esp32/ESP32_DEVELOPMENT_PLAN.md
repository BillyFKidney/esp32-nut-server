...
| `v2.8.Z` | `review/ota-browser-content-type` | Standing-approved scanner scope: reject browser ADMIN OTA-install bodies with missing or non-octet-stream content types before any OTA state/write/reboot work, while preserving ADMIN/CSRF precedence and established error contracts. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
