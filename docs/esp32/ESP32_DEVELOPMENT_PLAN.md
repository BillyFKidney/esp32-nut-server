...
| `v2.8.Z` | `review/device-route-json-escaping` | Standing-approved fixer scope: serialize accepted device-name success fields with the existing bounded JSON helpers so quotes and backslashes cannot break the response. Preserve persistence, response fields, and ADMIN/CSRF behavior. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
