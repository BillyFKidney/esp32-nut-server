...
`v2.8.0` through `v2.8.11` are published and target-tested. The next focused
...
| `v2.8.12` | `review/token-creation-workflow` | Token-workflow maintenance: consolidate duplicate private API/diagnostic token creation while retaining distinct stores, scopes, capacities, error contracts, ADMIN/CSRF, time gate, one-time plaintext responses, and zeroization. This reduces future drift risk between two security-sensitive issuance paths without broadening either credential's authority. Independent scan/review and an ESP-IDF v6.0.2 build passed; exact-tag build, scoped OTA, full token lifecycle/scope-isolation checks, and NUT acceptance remain required before publication. |
