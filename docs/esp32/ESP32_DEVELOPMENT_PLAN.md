...
| `v2.9.2` | Released | [PSRAM staged OTA](ESP32_V2_9_2_STAGED_OTA_CONTRACT.md): browser firmware upload writes the complete bounded image to PSRAM, validates it there, and leaves both OTA slots untouched until the ADMIN user explicitly clicks **Install Firmware**. The install action copies only the verified staged image to the inactive slot, selects it for boot, and restarts; failed, abandoned, malformed, or unverified uploads leave both OTA slots valid. The exact-tag artifact passed authenticated stage, malformed-upload rejection, explicit browser install, scoped Agent OTA, rollback-slot, runtime-PSRAM, service-boundary, and post-reboot NUT acceptance on 3Dprinter. [Evidence](../archive/v2.9.2/evidence.md). |
