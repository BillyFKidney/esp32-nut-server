...
| `v2.9.2` | `feature/psram-ota-receive-buffer` | PSRAM OTA receive buffer: replace the 4,096-byte OTA stack buffer with a bounded PSRAM allocation only after target proof that `httpd_req_recv()` and `esp_ota_write()` accept it; retain an explicit internal/DMA-capable fallback and every receive, abort, verification, partition, authorization, and reboot contract. Require malformed/upload/interruption acceptance, scoped OTA, rollback/recovery, exact-target size evidence, and post-reboot service/NUT acceptance. |
