...
| `v2.8.13` | `review/wifi-portal-lifecycle` | Completed code-review scope: extracted only the fallback captive-portal lifecycle from `wifi.c`, preserving AP+STA/open-AP/DHCP-DNS behavior, critical-section/event-bit ordering, failure cleanup, task lifetime, and all station/credential/physical recovery behavior. Independent review and the ESP-IDF build pass; release remains gated on a non-destructive setup-mode runtime acceptance. |
