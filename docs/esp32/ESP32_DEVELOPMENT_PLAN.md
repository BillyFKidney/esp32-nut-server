...
| `v2.8.15` | `review/wifi-credentials-storage` | Credential-storage maintenance: centralize the existing descriptor metadata without changing stored keys, schema, validation, zeroization, or provisioning behavior. Requires independent review, exact-tag build, scoped OTA, and normal-station service-boundary/full-NUT acceptance; no Wi-Fi credential mutation is required. |
