...
| `v2.9.5` | Released | [HID descriptor placement audit](../archive/v2.9.5/evidence.md): the maximum 500-entry `HIDData_t` array is 42,000 bytes, but it survives parsing and its entries are cached for recurring USB quick/full polling. The ordinary allocator policy remains in place rather than imposing strict PSRAM on this hot, long-lived structure; parser scratch and USB transfer/DMA buffers remain internal. Exact-tag build/size, scoped 3Dprinter OTA, USB-HID full poll, and service recovery acceptance passed before publication. |
