...
`v2.8.0` through `v2.8.5` are published and target-tested. The next focused
...
| `v2.8.Z` | `review/time-config-boundaries` | [Time-configuration code-review maintenance](ESP32_TIME_CONFIG_CONTRACT.md): split private NVS persistence and timezone policy from SNTP lifecycle and public operations without changing time, synchronization, or route contracts. Independent scan and contract are complete; implementation, review, exact-tag build, and live acceptance determine its assigned release number. |
