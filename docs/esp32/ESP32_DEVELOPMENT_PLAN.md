...
`v2.8.0` through `v2.8.7` are published and target-tested. The next focused
...
| `v2.8.Z` | `review/management-http-form-decoder` | Standing-approved fixer scope: extract the duplicate private form-component decode loops while preserving malformed-input, percent-decoding, plus-to-space, truncation, route, and security behavior. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
