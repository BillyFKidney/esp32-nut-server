...
| `v2.8.7` | `review/management-log-levels` | [Management-log code-review maintenance](ESP32_CURRENT_STATUS.md): centralized the duplicated private log-level mapping used by runtime-log capture and full-log routes while preserving every response name and payload contract. Independent review and ESP-IDF v6.0.2 build/size validation pass; exact-tag live acceptance remains before publication. |
