...
| `v2.8.Z` | `review/ota-route-contract` | [OTA route code-review maintenance](ESP32_OTA_ROUTE_CONTRACT.md): split the OTA request orchestration without changing authorization, image, persistence, response, partition, or reboot behavior. Independent scan and contract are complete; implementation, review, exact-tag build, and live acceptance determine its assigned release number. |
