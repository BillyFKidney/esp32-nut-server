...
`v2.8.0` through `v2.8.10` are published and target-tested. The next focused
...
| `v2.8.11` | `review/ota-browser-content-type` | Browser OTA boundary maintenance: reject missing or non-octet-stream ADMIN install bodies before any OTA state/write/reboot work. This makes malformed browser requests fail predictably and protects the inactive partition from unintended write attempts while preserving ADMIN/CSRF precedence and established error contracts. Independent scan/review and an ESP-IDF v6.0.2 build passed; exact-tag build, scoped OTA, browser rejection checks, and full NUT acceptance remain required before publication. |
