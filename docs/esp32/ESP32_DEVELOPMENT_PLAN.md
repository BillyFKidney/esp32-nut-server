...
| `v2.8.2` | `feature/token-store-refactor` | Close every remaining tracker item in [`.code-review-tracker.md`](../../.code-review-tracker.md): **CC-1** duplicated API-token/diagnostic-token create/delete flows, **CC-2** oversized create routines, and **O-23** the combined `api_tokens.c` responsibility. Refactor only after a reviewed storage-contract specification, focused tests for issue/list/delete/revocation behavior, bounded-resource evidence, and scope-separation regression checks. No new token capability or broader authorization is allowed. |
