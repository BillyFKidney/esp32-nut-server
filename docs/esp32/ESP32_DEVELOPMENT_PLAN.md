...
| `v2.8.Z` | `review/management-credentials-format` | Standing-approved fixer scope: reuse the private current-format credential predicate in verification while retaining the NVS-success guard and preserving schema, migration, iteration bounds, authentication, and zeroization behavior. Assign its concrete `v2.8.X` number only after independent review establishes the completed acceptance boundary. |
