...
| Field | Current fact |
| --- | --- |
| Active branch | `main` contains published [`v2.9.5`](https://github.com/BillyFKidney/esp32-nut-server/releases/tag/v2.9.5), completing the PSRAM placement program. It explicitly retains the maximum 42,000-byte HID descriptor array under ordinary allocator policy because cached entries are read on recurring USB quick/full polls; parser scratch and USB/DMA buffers stay internal. The exact 1,362,656-byte ESP-IDF v6.0.2 artifact SHA-256 is `104c75ee69849c65f440dc30c391a00444314810585458ad2dab0866f1c6c4b1`; 3Dprinter runs it in `app0` with `app1` next, authenticated 50-entry log/six-entry status acceptance, NUT `ok`/`OL`, `443`/`3493`, refused `8080`, and a 57-value `LIST VAR` poll. [Release evidence](../archive/v2.9.5/evidence.md) is complete. Exact next action: select a separate, evidence-scoped post-v2.9.5 improvement. |
