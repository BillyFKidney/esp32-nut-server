...
| Field | Current fact |
| --- | --- |
| Active branch | `main` is at the merged token-workflow review `a7da9970a`; the annotated release tag is `v2.8.12`. The next review remains unnumbered as `v2.8.Z` until its acceptance boundary is complete. |
| Full source-review coverage | All 73 tracked `src/**/*.c` files have a persisted reviewed, fixed, deferred, or clean outcome in [ESP32_SRC_CODE_REVIEW_COVERAGE.md](ESP32_SRC_CODE_REVIEW_COVERAGE.md). |
| Release preflight | All five v2.8.13–v2.8.17 candidate heads passed a fresh ESP-IDF v6.0.2 reconfigure/build/size check with 59% app-partition headroom. Both 1Password environments have matching direct/proxy certificate pins, pass pinned ADMIN-console validation, and have freshly mounted scoped bearer tokens: diagnostic status is authorized on both paths and OTA authorization reaches the expected content-type rejection before image processing. The remaining v2.8.13 acceptance gap is a credential-safe captive-portal runtime exercise. |
