...
| Field | Current fact |
| --- | --- |
| Active branch | `review/inherited-compatibility-fresh-scan` is a documentation-only, unversioned fresh bounded review of inherited compatibility debt. It confirms `src/common/strerror.c` needs a portability contract and fixtures before any structural change; no release candidate is assigned. |
