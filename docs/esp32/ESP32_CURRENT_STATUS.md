...
| Field | Current fact |
| --- | --- |
| Active branch | `review/inherited-compatibility-debt` is based on `main` at `f10f858c2`; it records ten freshly scanned inherited USB/HID/common/token-store items as explicit compatibility-contract debt, not unsafe hygiene fixes. |
