...
| Field | Current fact |
| --- | --- |
| Active branch | `review/nut-diagnostics-time-constant` is based on `main` at `f10f858c2`; it owns the reviewed `v2.8.16` diagnostic-time-unit candidate. The ESP-IDF build and independent regression review pass. Its release follows the pending v2.8.13–v2.8.15 candidates. |
