...
| Field | Current fact |
| --- | --- |
| Active branch | `release/v2.8.13-wifi-portal-lifecycle` is based on `main` at `f10f858c2`; it owns the reviewed `v2.8.13` fallback portal-lifecycle candidate. The ESP-IDF build and independent regression review pass; publication is gated on the authorized setup-mode runtime acceptance. |
