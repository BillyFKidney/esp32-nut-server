...
| Field | Current fact |
| --- | --- |
| Active branch | `release/v2.8.17-time-storage-import-order` contains the release-ready v2.8.17 time-storage import-order maintenance candidate. Fresh review of inherited-compatibility debt is the next unversioned scope. |
| v2.8.17 release candidate | This maintenance release aligns the standard-library include group in private time storage, making future persistence maintenance easier to audit without changing NVS schema, timezone behavior, or runtime behavior. The exact tagged ESP-IDF v6.0.2 build is 1,359,301 bytes with SHA-256 `460dac2f6f3f1af5804de79ee458319057954f25ae6fa7205507309785cc07da` and 59% app-slot headroom. It OTA-installed on 3Dprinter; diagnostics report `v2.8.17`, normal Wi-Fi, update `installed`, and full NUT acceptance reports UPS `OL`. Publication remains pending. |
