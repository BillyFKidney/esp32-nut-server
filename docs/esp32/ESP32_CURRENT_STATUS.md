...
| Field | Current fact |
| --- | --- |
| Active branch | `review/v2.8.z-ota-core` closes one bounded OTA-core silent-failure issue: after a queued success response, reboot-task allocation failure now immediately restarts into the selected verified image. The [OTA reboot-scheduling contract](ESP32_OTA_REBOOT_SCHEDULING_CONTRACT.md) preserves normal one-second restart, responses, NVS, image, partition, authorization, and check-only behavior. Scanner/fixer/reviewer completed cleanly; the clean ESP-IDF v6.0.2 candidate build embedded `v2.8.17-10-gd04243886` with SHA-256 `3a69e2b096fb013e2508d0fa80a64823f41efa9693367f120449f2bf25a183fb` and 59% app-slot headroom. Certificate-pinned scoped OTA installed it on 3Dprinter into `app0`; post-reboot diagnostics report `installed`, NUT `ok`, and UPS `OL`, with HTTPS `443`/NUT `3493` open and `8080` refused. The exact next action is merge readiness, then a concrete release version and explicit tag/publication authorization. |
