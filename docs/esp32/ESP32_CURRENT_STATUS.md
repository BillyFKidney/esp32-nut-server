...
| Field | Current fact |
| --- | --- |
| Active branch | `review/v2.8.z-management-diagnostics-routes` contains a fresh, clean full-rubric scan of the ESP32-executed diagnostic routes; stale historical concerns do not exist in current code, so no fixer pass or release candidate is warranted. `main` retains the completed, unversioned `strerror()` portability contract, which is protected inherited compatibility coverage rather than a release candidate or planned refactor. The exact next action is to select another fresh ESP32-executed review target. |
