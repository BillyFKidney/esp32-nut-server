...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/v2.11-code-quality` at `2e90dec1e`, tracking `origin/feature/v2.11-code-quality` with no divergence; it is stacked on `reorg/esp32-docs-ci-quarantine` at `758bbf944`. The published `main` baseline remains `e5cc55ce6`. |
...
| v2.11 code-quality candidate validation | On September 9, the clean ESP-IDF v6.0.2 build of `2e90dec1e` produced `build/nut-esp32s3.bin` (1,362,080 bytes, SHA-256 `da1723ce5fffd81f93f5374520867ad5ac048e946a576c8d4fa7aacb5daf1323`, 59% app-slot headroom) with development identity `v2.8.0-4-g2e90dec1e`. The approved 3Dprinter Agent Tests unit accepted it through the fingerprint-verified scoped OTA route and rebooted into `app1`. Post-reboot authenticated Agent status reports that exact identity, `last_result: installed`, HTTPS `443`, read-only NUT `3493`, refused `8080`, `nut.health: ok`, and a complete 57-variable NUT poll with `ups.status OL`. Browser proxy validation is blocked: the configured 3Dprinter FQDN resolves to `192.168.40.89` but Chrome and curl receive connection refusal; the device itself reports refreshed Wi-Fi address `192.168.40.88` and direct verified HTTPS is healthy. No v2.11 tag or release is authorized. |
...
Next exact action: review the stacked `reorg/esp32-docs-ci-quarantine` and
`feature/v2.11-code-quality` branches, then decide whether to merge the
code-quality candidate and separately repair the 3Dprinter reverse-proxy
listener before browser acceptance. Logs Copy/Download and clipboard fallback
remain untested, and Garage remains untouched until it is independently
reachable and healthy after the September 2 power blink.
