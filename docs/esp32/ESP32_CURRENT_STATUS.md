...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/psram-management-log-retention` holds annotated `v2.9.3`, ready to publish. It moves the 50-entry retained management-log ring (10,800 bytes) to strict PSRAM and removes the 5,184-byte static internal ring while keeping hot capture state and lock metadata internal. The exact 1,362,656-byte ESP-IDF v6.0.2 artifact SHA-256 is `a1cb117f76c825bcdc368ece1d4989e204d60e9e4c3a82d5c0b910f1593ca4c7`; 3Dprinter runs it in `app1` with `app0` rollback, authenticated 50-entry log/six-entry status acceptance, NUT `ok`/`OL`, `443`/`3493`, refused `8080`, and a 57-value `LIST VAR` poll. [Release evidence](../archive/v2.9.3/evidence.md) is complete. Exact next action: publish `v2.9.3`, merge it to `main`, then audit v2.9.4 auth/lock placement. |
