...
| Field | Current fact |
| --- | --- |
| Active branch | `review/wifi-credentials-storage` is based on `main` at `f10f858c2`; it owns the reviewed `v2.8.15` Wi-Fi credential storage candidate. The ESP-IDF build and independent regression review pass. Its release follows the pending v2.8.13 and v2.8.14 candidates. |
