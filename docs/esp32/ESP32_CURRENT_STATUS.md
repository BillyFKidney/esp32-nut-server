...
| Field | Current fact |
| --- | --- |
| Active branch | `release/v2.8.16-nut-diagnostics-time` contains the release-ready v2.8.16 diagnostic-time maintenance candidate. The next isolated candidate is v2.8.17 time-storage import ordering. |
| v2.8.16 release candidate | This maintenance release names the diagnostic seconds-to-microseconds conversion, reducing the chance of future time-unit mistakes without changing disconnect duration bounds, RAM-only state, diagnostic routes, or payloads. The exact-tag ESP-IDF v6.0.2 publication build is 1,359,301 bytes with SHA-256 `dbc6d441918bb0b2579012b8fa4f06db9a251ea3e11d07110a4482405b36e065` and 59% app-slot headroom. It OTA-installed on 3Dprinter; diagnostics report `v2.8.16`, normal Wi-Fi, update `installed`, and healthy NUT. Release publication remains pending. |
