...
| Field | Current fact |
| --- | --- |
| Active branch | `review/dns-server-lifecycle` is based on `main` at `f10f858c2`; it owns the reviewed `v2.8.14` captive-DNS lifecycle candidate. The ESP-IDF build and independent regression review pass. Its release follows the pending `v2.8.13` portal-lifecycle candidate. |
