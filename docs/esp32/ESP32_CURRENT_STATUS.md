...
| Agent Tests preflight | Observed 2026-09-11: direct HTTPS responds, but both stored certificate public-key pins mismatch and both stored bearer tokens return `401`. Reconcile the 1Password Agent Tests TLS/token metadata before OTA or service acceptance. No serial device is enumerated; captive-portal runtime acceptance remains not tested. |
