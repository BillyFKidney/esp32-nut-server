...
| Field | Current fact |
| --- | --- |
| Active branch | `review/management-http-form-decoder` is based on `main` at `179d37ee5`; it owns one standing-approved v2.8.Z form-component-decoder review item. Device-configuration boundaries scanned clean and are not a release slice. |
...
The next preparation scan found `src/management-device-config.c` clean across
...
authorizes extracting that private decoder while preserving every current
malformed-input, percent-decoding, plus-to-space, truncation, route, and
security behavior. The exact next action is the standing-approved fixer.
