...
| Field | Current fact |
| --- | --- |
| Active branch | `review/time-storage-import-order` is based on `main` at `f10f858c2`; it owns the reviewed `v2.8.17` time-storage candidate. The ESP-IDF build and independent regression review pass. Its release follows the pending v2.8.13–v2.8.16 candidates. |
| Device-environment preflight | Observed 2026-09-11: Agent Tests and Garage direct HTTPS respond, but their stored certificate public-key pins mismatch and their diagnostic bearer tokens return `401`. Reconcile both 1Password TLS/token records before OTA or service acceptance. No serial device is enumerated; captive-portal runtime acceptance remains not tested. |
