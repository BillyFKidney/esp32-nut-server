...
| Release preflight | All five v2.8.13–v2.8.17 candidate heads passed a fresh ESP-IDF v6.0.2 reconfigure/build/size check with 59% app-partition headroom. Both 1Password environments have matching direct/proxy certificate pins and pass pinned ADMIN-console validation, but their scoped API and diagnostic bearer tokens return 401 and must be securely replaced before OTA acceptance; v2.8.13 also needs a credential-safe captive-portal runtime exercise. |
