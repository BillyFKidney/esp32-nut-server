...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/psram-wifi-scan-records` contains exact-tag `v2.9.1`, which places the bounded 20-record Wi-Fi driver output array in PSRAM first for both authenticated and portal scans. The exact ESP-IDF v6.0.2 artifact is 1,359,309 bytes with SHA-256 `3d0f0478bb6ff9445cd42341d62f6285fd1609f0bcf227223bb3c1598935268d`; it OTA-installed on 3Dprinter in `app0`, reports `v2.9.1` and update `installed`, and passed its authenticated 20-network-capped scan. NUT is `ok` with UPS `OL`; HTTPS `443` and NUT `3493` responded, `8080` was refused, and the read-only NUT poll returned 57 variables. [Release evidence](../archive/v2.9.1/evidence.md) is complete. The exact next action is the authorized merge and publication, followed by `v2.9.2` staged-PSRAM OTA design. |
