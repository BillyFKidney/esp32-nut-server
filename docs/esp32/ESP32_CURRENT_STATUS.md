...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/psram-wifi-scan-records` derives from `main` at released [`v2.9.0`](https://github.com/BillyFKidney/esp32-nut-server/releases/tag/v2.9.0), which explicitly places the bounded 7,000-byte status response and 24-entry full-log snapshot in PSRAM. The exact ESP-IDF v6.0.2 artifact is 1,359,285 bytes with SHA-256 `a116275bbb0b24cacf1155a20e8444a7568f47f8789a7168c730e7858e375f0c`; it OTA-installed on 3Dprinter in `app0`, reports `v2.9.0` and update `installed`, NUT `ok`, and UPS `OL`. HTTPS `443` and NUT `3493` responded while `8080` was refused. [Release evidence](../archive/v2.9.0/evidence.md) is complete. The active next action is the bounded `v2.9.1` Wi-Fi scan-record PSRAM migration. |
