...
| Field | Current fact |
| --- | --- |
| Active branch | `main` contains the merged OTA reboot-scheduling fix: after a queued success response, reboot-task allocation failure immediately restarts into the selected verified image. Scanner/fixer/reviewer and certificate-pinned 3Dprinter candidate acceptance are clean; the recorded candidate build embedded `v2.8.17-10-gd04243886`, had SHA-256 `3a69e2b096fb013e2508d0fa80a64823f41efa9693367f120449f2bf25a183fb`, and recovered with update `installed`, NUT `ok`, and UPS `OL`. [v2.8.18 release preparation](../archive/v2.8.18/evidence.md) is authorized. The exact next action is the annotated tag, exact-tag build, 3Dprinter OTA acceptance, and publication. |
