...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/psram-staged-ota` holds annotated `v2.9.2`, ready to publish. It stages authenticated browser firmware only in PSRAM until explicit install, preserving both slots through check and rejection; the exact 1,362,384-byte ESP-IDF v6.0.2 artifact SHA-256 is `6845cbf094fed80eef29a28b40a6446c915bb77d96dea416fe5e9bff20e66abf`. It browser-staged, rejected a malformed replacement without changing slots, then browser-installed on 3Dprinter into `app1` with `app0` rollback; NUT is `ok`/`OL`, `443`/`3493` respond, `8080` is refused, and `LIST VAR` returned 57 values. [Release evidence](../archive/v2.9.2/evidence.md) is complete. Exact next action: publish `v2.9.2`, merge it to `main`, then select the next PSRAM-plan group. |
