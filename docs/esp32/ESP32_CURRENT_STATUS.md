...
| Field | Current fact |
| --- | --- |
| Active branch | `main` contains the released [`v2.8.18`](https://github.com/BillyFKidney/esp32-nut-server/releases/tag/v2.8.18) OTA reboot-scheduling fix: after a queued success response, reboot-task allocation failure immediately restarts into the selected verified image. The exact ESP-IDF v6.0.2 build embedded `v2.8.18`, had SHA-256 `faa4409567e39b700714bd092226cf9390d132b3ff99fa08340a44144f1a5159`, 59% app-slot headroom, and OTA-installed on 3Dprinter. It recovered in `app1` with update `installed`, NUT `ok`, and UPS `OL`; HTTPS `443` and NUT `3493` responded, while `8080` was refused. A read-only Garage diagnosis found the NUT listener healthy but its USB-HID UPS peer repeatedly resetting; do not reset or OTA Garage before a physical USB path check. [Release evidence](../archive/v2.8.18/evidence.md) is complete. The exact next action is selection of the next bounded review. |
