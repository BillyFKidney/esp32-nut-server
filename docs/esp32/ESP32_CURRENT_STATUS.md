...
| Field | Current fact |
| --- | --- |
| Active branch | `feature/v2.8.1-code-quality` is the active maintenance candidate, stacked on `reorg/esp32-docs-ci-quarantine`; its exact commit and remote divergence are refreshed at release closeout. The published `main` baseline remains `e5cc55ce6`. |
...
| v2.8.1 code-quality candidate | This maintenance slice contains no product features: it preserves the released management, authorization, NUT, UPS, and service contracts while applying narrowly reviewed code-quality improvements. The September 9 pre-version-correction candidate was OTA-installed successfully and passed direct HTTPS/NUT acceptance; its evidence is superseded by the required clean v2.8.1 candidate and exact-tag validation in [archive/v2.8.1/evidence.md](archive/v2.8.1/evidence.md). The repaired 3Dprinter browser proxy must pass FQDN acceptance before publication. |
...
Next exact action: build and validate the v2.8.1 code-quality candidate through
the repaired 3Dprinter FQDN and direct service boundaries, then complete the
authorized tag and GitHub release only if all acceptance checks pass. Logs
Copy/Download and clipboard fallback remain untested, and Garage remains
untouched until it is independently reachable and healthy after the September
2 power blink.
