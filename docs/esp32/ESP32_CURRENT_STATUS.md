...
| Field | Current fact |
| --- | --- |
| Active branch | `main` contains the completed, unversioned `strerror()` portability contract, forced-fallback fixture, and macOS/Ubuntu CI matrix. Both CI jobs passed, and 3Dprinter reports v2.8.17 with healthy normal-libc/NUT service. `strerror.c` is protected inherited compatibility code, not a release candidate or planned refactor. The exact next action is to select a fresh review target that executes on ESP32. |
