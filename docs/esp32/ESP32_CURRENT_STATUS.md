...
| Field | Current fact |
| --- | --- |
| Active branch | `main` contains the tagged `v2.8.18` OTA reboot-scheduling fix: after a queued success response, reboot-task allocation failure immediately restarts into the selected verified image. The exact ESP-IDF v6.0.2 build embedded `v2.8.18`, had SHA-256 `faa4409567e39b700714bd092226cf9390d132b3ff99fa08340a44144f1a5159`, 59% app-slot headroom, and OTA-installed on 3Dprinter. It recovered in `app1` with update `installed`, NUT `ok`, and UPS `OL`; HTTPS `443` and NUT `3493` responded, while `8080` was refused. [v2.8.18 release evidence](../archive/v2.8.18/evidence.md) is complete; the exact next action is publication, then a read-only Garage stale-NUT diagnosis. |
