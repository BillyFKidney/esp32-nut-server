...
- For v2.7.2+ diagnostic API checks, use the fingerprint-pinned token probe
  with the scope-specific credential retrieved through the 1Password MCP
  developer environment. Confirm the simulated stale result and recovery before
  the separately authorized physical USB disconnect; simulation is not a
  substitute for that final acceptance test.
