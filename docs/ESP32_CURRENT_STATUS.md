...
| v2.7.11 target state | Clean candidate `7a1239084` was built as `v2.7.10-3-g7a1239084` and OTA-installed on the authorized `3Dprinter` unit. It is running from `app1`, reports update state `installed`, and recovered to a full NUT poll with `health: ok` and UPS `OL`. Browser testing now exposes proxy `HTTP 413` accurately; the Synology host-specific upload-limit/timeout change remains pending before successful browser check/install acceptance. |
...
Next exact action: apply the documented host-specific Synology proxy upload
bounds for `esp32nut-3dprinter.28670avenidacondesa.com`, then repeat browser
check/install acceptance on the authorized `3Dprinter` candidate.
