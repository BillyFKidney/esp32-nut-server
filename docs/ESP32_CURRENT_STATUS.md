...
| Field | Current fact |
| --- | --- |
| Active branch | `main` at merge commit `baec0c729`; the annotated `v2.8.0` tag points to this merge. |
...
Next exact action: plan the authorized `v3.0.0` API v2 and token-contract slice;
Logs Copy/Download and clipboard fallback remain untested, and Garage remains
untouched until it is independently reachable and healthy after the September 2
power blink.
